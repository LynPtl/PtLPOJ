...
* [x] **6.1 端到端联调:** 在内网环境下跑通“登录 -> 拉题 -> 提交 -> 返回结果”的完整主干链路 (Happy Path)。
* [x] **6.2 异常用例测试:** 故意提交包含死循环、内存泄漏、恶意系统调用的 Python 代码，验证 Sandbox 的熔断机制。
* [x] **6.3 压力测试:** 模拟多用户并发提交，验证 Go 后端 Channel 队列的背压（Backpressure）表现。
